...
{
    public class SangioveseBoilerProjectile : ModProjectile
    {
        ...
        {
            if (SangioveseBoiler.ShootType == 0)
            {
        ...
        SoundStyle VineBoomSound = new SoundStyle($"{nameof(arkimedeezMod)}/Assets/Audio/Gunshot1");
        bool Gunshot = false;

        public override void AI()
        {
            int x = Main.rand.Next(1, 8);
            if (x == 2)
            {
            ...
            if (Gunshot == false)
            {
                SoundEngine.PlaySound(VineBoomSound with { Volume = 0.3f }, Main.player[Projectile.owner].position);
                Gunshot = true;
            }
        }
