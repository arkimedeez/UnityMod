    ...
    public class StarSeekerSwing : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.rand.NextFloat() * 2f < Projectile.Opacity)
            {
                ...
                Color dustColor = Color.Lerp(Color.LightPink, Color.DeepPink, Main.rand.NextFloat() * 0.3f);
            ...
            if (Main.rand.NextFloat() * 1.5f < Projectile.Opacity)
            {
                ...
                Dust.NewDustPerfect(dustPosition, DustID.TintableDustLighted, dustVelocity, 100, Color.HotPink * Projectile.Opacity, 1.2f * Projectile.Opacity);
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Color backDarkColor = new Color(108, 23, 69); // Original Excalibur color: Color(180, 160, 60)
            Color middleMediumColor = new Color(0, 145, 189); // Original Excalibur color: Color(255, 255, 80)
            Color frontLightColor = new Color(243, 117, 168); // Original Excalibur color: Color(255, 240, 150)
            ...
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.Pink * 0.6f * lerpTime, Projectile.rotation + Projectile.ai[0] * 0.01f, origin, scale, spriteEffects, 0f);
            // Thin middle line (final frame)
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.Pink * 0.5f * lerpTime, Projectile.rotation + Projectile.ai[0] * -0.05f, origin, scale * 0.8f, spriteEffects, 0f);
            // Thin bottom line (final frame)
            Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.Pink * 0.4f * lerpTime, Projectile.rotation + Projectile.ai[0] * -0.1f, origin, scale * 0.6f, spriteEffects, 0f);
